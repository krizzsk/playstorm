...
public class BidTokenEncoder {
    ...
    private String constructV3Token(String str, int i, int i2) {
        return this.gson.toJson((Object) new BidTokenV3(new Device(Boolean.valueOf(this.platform.getIsBatterySaverEnabled()), this.localeInfo.getLanguage(), this.localeInfo.getTimeZoneId(), Double.valueOf(this.platform.getVolumeLevel()), new Extension(Boolean.valueOf(this.platform.getIsSideloaded()), Boolean.valueOf(this.platform.getIsSDCardPresent()), Boolean.valueOf(this.platform.getIsSoundEnabled()))), new Request(getConfigExtension(), Integer.valueOf(i2), getAvailableBidTokens(str, i, i2), VungleApiClient.getHeaderUa()), new Consent(new Ccpa(getCCPAStatus(this.repository.load(Cookie.CCPA_COOKIE, Cookie.class).get(this.timeoutProvider.getTimeout(), TimeUnit.MILLISECONDS))), getGdpr(), getCoppa())));
    }
