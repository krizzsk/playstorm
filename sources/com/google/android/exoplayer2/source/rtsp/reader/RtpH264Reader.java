...
final class RtpH264Reader implements RtpPayloadReader {
    ...
    public void consume(ParsableByteArray parsableByteArray, long j, int i, boolean z) throws ParserException {
        int i2 = i;
        try {
            byte b = parsableByteArray.getData()[0] & Ascii.f17947US;
    ...
    private void processSingleNalUnitPacket(ParsableByteArray parsableByteArray) {
        ...
        this.bufferFlags = getBufferFlagsFromNalType(parsableByteArray.getData()[0] & Ascii.f17947US);
    }
    ...
    private void processFragmentationUnitPacket(ParsableByteArray parsableByteArray, int i) {
        ...
        byte b3 = (b & 224) | (b2 & Ascii.f17947US);
        boolean z = (b2 & 128) > 0;
        ...
        if (z) {
            ...
            if (i != i2) {
                Log.m16949w(TAG, Util.formatInvariant("Received RTP packet with unexpected sequence number. Expected: %d; received: %d. Dropping packet.", Integer.valueOf(i2), Integer.valueOf(i)));
                return;
            } else {
                this.fuScratchBuffer.reset(parsableByteArray.getData());
        ...
        this.fragmentedSampleSizeBytes += bytesLeft;
        if (z2) {
            this.bufferFlags = getBufferFlagsFromNalType(b3 & Ascii.f17947US);
        }
    }
