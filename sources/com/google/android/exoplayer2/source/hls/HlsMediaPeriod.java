...
public final class HlsMediaPeriod implements MediaPeriod, HlsSampleStreamWrapper.Callback, HlsPlaylistTracker.PlaylistEventListener {
    ...
    private void buildAndPrepareMainSampleStreamWrapper(HlsMasterPlaylist hlsMasterPlaylist, long j, List<HlsSampleStreamWrapper> list, List<int[]> list2, Map<String, DrmInitData> map) {
        ...
        int i = 0;
        int i2 = 0;
        for (int i3 = 0; i3 < hlsMasterPlaylist2.variants.size(); i3++) {
            Format format = hlsMasterPlaylist2.variants.get(i3).format;
        ...
        int i4 = 0;
        for (int i5 = 0; i5 < hlsMasterPlaylist2.variants.size(); i5++) {
            if ((!z2 || iArr[i5] == 2) && (!z || iArr[i5] != 1)) {
                HlsMasterPlaylist.Variant variant = hlsMasterPlaylist2.variants.get(i5);
        ...
        HlsSampleStreamWrapper buildSampleStreamWrapper = buildSampleStreamWrapper((z2 || codecCountOfType2 <= 0) ? 0 : 1, uriArr, formatArr, hlsMasterPlaylist2.muxedAudioFormat, hlsMasterPlaylist2.muxedCaptionFormats, map, j);
    ...
    private static Format deriveVideoFormat(Format format) {
        ...
        return new Format.Builder().setId(format.f14457id).setLabel(format.label).setContainerMimeType(format.containerMimeType).setSampleMimeType(MimeTypes.getMediaMimeType(codecsOfType)).setCodecs(codecsOfType).setMetadata(format.metadata).setAverageBitrate(format.averageBitrate).setPeakBitrate(format.peakBitrate).setWidth(format.width).setHeight(format.height).setFrameRate(format.frameRate).setSelectionFlags(format.selectionFlags).setRoleFlags(format.roleFlags).build();
    }
    ...
    private static Format deriveAudioFormat(Format format, Format format2, boolean z) {
        ...
        return new Format.Builder().setId(format.f14457id).setLabel(str).setContainerMimeType(format.containerMimeType).setSampleMimeType(mediaMimeType).setCodecs(str3).setMetadata(metadata).setAverageBitrate(i12).setPeakBitrate(i4).setChannelCount(i).setSelectionFlags(i2).setRoleFlags(i3).setLanguage(str2).build();
    }
}
