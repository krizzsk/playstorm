...
final class ReportQueue {
    ...
    ReportQueue(double d, double d2, long j, Transport<CrashlyticsReport> transport2, OnDemandCounter onDemandCounter2) {
        ...
        this.singleThreadExecutor = new ThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, this.queue);
        this.step = 0;
        ...
        synchronized (this.queue) {
            ...
            if (z) {
                ...
                if (isQueueAvailable()) {
                    ...
                    logger.mo51961d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                    Logger logger2 = Logger.getLogger();
                    logger2.mo51961d("Queue size: " + this.queue.size());
                    this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                    Logger logger3 = Logger.getLogger();
                    logger3.mo51961d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                    taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                    return taskCompletionSource;
                }
                calcStep();
                Logger logger4 = Logger.getLogger();
                logger4.mo51961d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void sendReport(CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, TaskCompletionSource<CrashlyticsReportWithSessionId> taskCompletionSource) {
        Logger logger = Logger.getLogger();
        logger.mo51961d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
        this.transport.schedule(Event.ofUrgent(crashlyticsReportWithSessionId.getReport()), new TransportScheduleCallback(crashlyticsReportWithSessionId) {
            ...
            public final void onSchedule(Exception exc) {
                ReportQueue.lambda$sendReport$0(TaskCompletionSource.this, this.f$1, exc);
            }
        });
    }
    ...
    private final class ReportRunnable implements Runnable {
        ...
        public void run() {
            ...
            Logger logger = Logger.getLogger();
            logger.mo51961d("Delay for: " + String.format(Locale.US, "%.2f", new Object[]{Double.valueOf(access$300 / 1000.0d)}) + " s for report: " + this.reportWithSessionId.getSessionId());
